...
private:
  ...
  static void _imu_pointer_read(lv_indev_drv_t *indev_drv,
                                lv_indev_data_t *data) {
    if (_imu->accelerationAvailable()) {
      float x, y, z;
      int ret = _imu->readAcceleration(x, y, z);
      ESP_LOGI(TAG, "_imu_pointer_read() ret=%d %f,%f,%f", ret, x, y, z);
      // 左上角偏: x,y = 0.570312,-0.530884
      // x: 从 0.5 -> -0.5 mapping 到 0.0 -> 1.0
      // y: 从 -0.5 -> 0.5 mapping 到 0.0 -> 1.0
      float x_percent = 0.5 - x;
      if (x_percent > 1)
        x_percent = 1;
      if (x_percent < 0)
        x_percent = 0;
      float y_percent = 0.5 + y;
      if (y_percent > 1)
        y_percent = 1;
      if (y_percent < 0)
        y_percent = 0;
      data->point.x = lv_coord_t(_disp->width() * x_percent);
      data->point.y = lv_coord_t(_disp->height() * y_percent);
      data->state = LV_INDEV_STATE_PR;
    } else {
      data->state = LV_INDEV_STATE_REL;
    }
  ...
  inline void _lv_port_disp_init() {
    ...
    buf_3_1 = (lv_color_t *)malloc(_disp->width() * _disp->height() *
                                   sizeof(lv_color_t));
    buf_3_2 = (lv_color_t *)malloc(_disp->width() * _disp->height() *
                                   sizeof(lv_color_t));
#else
    buf_3_1 = (lv_color_t *)heap_caps_malloc(_disp->width() * _disp->height() *
                                                 sizeof(lv_color_t),
                                             MALLOC_CAP_SPIRAM);
    buf_3_2 = (lv_color_t *)heap_caps_malloc(_disp->width() * _disp->height() *
                                                 sizeof(lv_color_t),
                                             MALLOC_CAP_SPIRAM);
#endif
    /* If failed */
  ...
  inline void _lv_port_indev_init() {
...
#if USE_IMU_POINTER == 1
    {
      ...
      lv_img_set_src(imu_cursor, LV_SYMBOL_CLOSE);
...
public:
   ...
   * @param tp
   */
  inline void lv_port_init(lgfx::LGFX_Device *disp, FT3168::TP_FT3168 *tp,
                           BMI270::BMI270 *imu) {
    _disp = disp;
