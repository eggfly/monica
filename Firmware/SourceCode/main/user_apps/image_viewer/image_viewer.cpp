...
namespace UserApps {
        ...
        void Sketchpad::onCreate()
        {
            ...
            *_data.key_home_ptr = false;
            ...
            // eggfly
            lv_obj_t* img = lv_img_create(_data.screen);
            lv_obj_center(img);
            lv_img_header_t header;
            lv_res_t res = lv_img_decoder_get_info("A:sdcard/pencil_600.png", &header);
            if(res == LV_RES_OK) {
                ESP_LOGI(TAG, "decoder: w,h=%d,%d", header.w, header.h);
                lv_img_set_src(img, "A:sdcard/pencil_600.png");
                if (header.w > 360 && header.h > 360) {
                    uint16_t scale = std::min(LV_IMG_ZOOM_NONE * 360 / header.w, LV_IMG_ZOOM_NONE * 360 / header.h);
