...
//----------------------------------------------------------------------------
  ...
  void Bus_QSPI::config(const config_t& cfg)
  {
    _cfg = cfg;
...
//ESP_LOGI("LGFX","Bus_QSPI::config  spi_port:%d  dc:%0d %02x", spi_port, _cfg.pin_dc, _mask_reg_dc);
  }
...
//ESP_LOGI("LGFX","Bus_QSPI::release");
  ...
  void Bus_QSPI::endTransaction(void)
  {
    dc_control(true);
  ...
  bool Bus_QSPI::writeCommand(uint32_t data, uint_fast8_t bit_length)
  {
...
/// ESP32-C3 で HIGHPART を使用すると異常動作するため分岐する;
  ...
  void Bus_QSPI::addDMAQueue(const uint8_t* data, uint32_t length)
  {
    if (!_cfg.dma_channel)
    {
...
label_start:
  ...
  void Bus_QSPI::beginRead(uint_fast8_t dummy_bits)
  {
    beginRead();
  ...
  uint32_t Bus_QSPI::readData(uint_fast8_t bit_length)
  {
  ...
  bool Bus_QSPI::readBytes(uint8_t* dst, uint32_t length, bool use_dma)
  {
...
/// ESP32-C3 で HIGHPART を使用すると異常動作するため分岐する;
  ...
  void Bus_QSPI::readPixels(void* dst, pixelcopy_t* param, uint32_t length)
  {
...
/// ESP32-C3 で HIGHPART を使用すると異常動作するため分岐する;
  ...
  void Bus_QSPI::_alloc_dmadesc(size_t len)
  {
  ...
  void Bus_QSPI::_spi_dma_reset(void)
  {
#if defined( SOC_GDMA_SUPPORTED )  // for C3/S3
  ...
  void Bus_QSPI::_setup_dma_desc_links(const uint8_t *data, int32_t len)
