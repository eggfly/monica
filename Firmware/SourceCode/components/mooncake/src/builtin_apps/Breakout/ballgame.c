...
void ballgame_start() {
  ...
  lv_obj_add_event_cb(botton_exit, ballgame_all_clear, LV_EVENT_RELEASED, 0);
...
void ready_cb(lv_anim_t *var) { lv_obj_del((lv_obj_t *)var->var); }

void cube_anim_great(lv_color_t value, short x, short y) {
  int i;
  lv_anim_t a[6];
  lv_obj_t *cube_split[6];
  lv_anim_exec_xcb_t lv_anim_exec_xcb[6] = {
      exec_cbx1, exec_cbx2, exec_cbx3, exec_cbx4, exec_cbx5, exec_cbx6,
  };
...
static int obj_is_crash(lv_obj_t *obj1, lv_obj_t *obj2) {
  ...
  if ((x2 - x1) == w1 && (y2 - y1) <= (h1 - 3) && (y1 - y2) <= h2 - 3) {
    return 1;
  }
  if ((y1 - y2) == h2 && (x2 - x1) <= (w1 - 3) && (x1 - x2) <= w2 - 3) {
    return 2;
  }
  if ((x1 - x2) == w2 && (y2 - y1) <= (h1 - 3) && (y1 - y2) <= h2 - 3) {
    return 3;
  }
  if ((y2 - y1) == h1 && (x2 - x1) <= (w1 - 3) && (x1 - x2) <= w2 - 3) {
    return 4;
  }
  return 0;
}
...
void timer_cb1(lv_timer_t *t) {
  ...
  if (endx <= 0 || endx >= 479 - ballsizie) {
    endx -= movx;
    currenty -= movy;
    movx = movx * (-1);
  }
  if (currenty <= 0 || currenty >= 319 - ballsizie) {
    endx -= movx;
    currenty -= movy;
    movy = movy * (-1);
  }
    ...
    for (int j = 0; j < nlie; j++) {
      if (cube[i][j].alive) {
        if (obj_is_crash(qiu1, cube[i][j].obj) == 1) {
          score++;
          cube_anim_great(
              lv_obj_get_style_bg_color(cube[i][j].obj, LV_PART_MAIN),
              lv_obj_get_x(cube[i][j].obj), lv_obj_get_y(cube[i][j].obj));
          cube[i][j].alive--;
...
void btn1_event_cb(lv_event_t *e) {
  ...
  for (int i = 0; i < nhang; i++) {
    for (int j = 0; j < nlie; j++) {
      ...
      lv_obj_set_pos(cube[i][j].obj,
                     j * (cubeendx - cubestartx) / nlie + cubestartx,
                     i * (cubeendy - cubestarty) / nhang + cubestarty);
      lv_obj_set_style_bg_color(cube[i][j].obj,
