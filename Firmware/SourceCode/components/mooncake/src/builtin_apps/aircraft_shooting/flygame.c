...
void fly_game_start(void) {
  ...
  lv_obj_add_event_cb(botton_exit, flygame_all_clear, LV_EVENT_RELEASED, 0);
  ...
  lv_obj_set_style_text_color(score_lable, lv_color_hex(0x00ffff),
                              LV_PART_MAIN);
...
void timer_cb3(lv_timer_t *t) {
  ...
  if (score > 5000 && enemy_boss.alive == 0) {
    ...
    lv_obj_set_style_bg_color(boss_live, lv_color_hex(0xff0000),
                              LV_PART_INDICATOR);
  ...
  for (i = 0; i < zidanshuliang; i++) {
    if (zidan[i].alive == 1) {
      ...
      for (j = 0; j < enemyshuliang; j++) {
        if (enemy[j].alive == 1) {
          if ((zidan[i].x - enemy[j].x < 40 && zidan[i].x - enemy[j].x > -20) &&
              (zidan[i].y - enemy[j].y < 20 && zidan[i].y - enemy[j].y > -30)) {
            lv_obj_del(zidan[i].obj);
            zidan[i].alive = 0;
  ...
  for (i = 0; i < zidanshuliang; i++) {
    if (zidan[i].alive == 1) {
      ...
      if (enemy_boss.alive == 1) {
        if ((zidan[i].x - enemy_boss.x < 50 &&
             zidan[i].x - enemy_boss.x > -50) &&
            (zidan[i].y - enemy_boss.y < 50 &&
             zidan[i].y - enemy_boss.y > -50)) {
          lv_obj_del(zidan[i].obj);
          zidan[i].alive = 0;
          boss_live_value--;
